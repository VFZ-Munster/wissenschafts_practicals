...
resources:
  ...
  - recap/logistic_regression_recap/logistic_regression_interactive_recap.html
...
::: {.download-card}
[See the recap for Practical 6](recap/logistic_regression_recap/logistic_regression_interactive_recap.html){target="_blank" rel="noopener"}
:::
...
::: {.download-card}
[Zusammenfassung zu Praktikum 6 ansehen](recap/logistic_regression_recap/logistic_regression_interactive_recap.html){target="_blank" rel="noopener"}
:::
...
| Variable | Coefficient B | Sig. | Exp(B) |
|---|---|---|---|
| bmi | 0.011 | 0.027 | 1.011 |
| age | 0.037 | <0.001 | 1.038 |
| Former | 0.348 | <0.001 | 1.416 |
| Current | 1.190 | <0.001 | 3.287 |
| Constant | −4.109 | <0.001 | 0.016 |
...
In Exercise 1.1, `bmi` alone was **not** significantly associated with periodontitis (p = 0.751, Exp(B) ≈ 1). In the multivariable model above, `bmi` becomes statistically significant (p = 0.027). This happens because `age` and `bmi` are correlated with each other and both affect periodontitis risk — adjusting for `age` removes some of that confounding and reveals the effect of `bmi` more clearly.
:::
